...
class SpecialCredit(User):
    # with Special credit, we can rent a book for 20 days and can only rent 10 books
    # if we return book next 21 days or more, our account will block for 2 days
    _rentLimit = 10  # books
    _blockTime = 2   # days  
    _returnTime = 20 # days
    def __init__(self, Id, name, password):
        super().__init__(Id, name, password, "Special")
...
class NormalCredit(SpecialCredit):
    # with Normal credit, we can rent a book for 10 days and can only rent 5 books
    # if we return book next 11 days or more, our account will block for 3 days
    _rentLimit = 5   # books
    _blockTime = 3   # days 
    _returnTime = 10 # days
    def __init__(self, Id, name, password):
        User.__init__(self, Id, name, password, "Normal")
